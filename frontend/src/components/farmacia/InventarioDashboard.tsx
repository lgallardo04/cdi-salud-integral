...
export const InventarioDashboard: React.FC = () => {
  ...
  const { addToast, createItem } = useData();

  const handleEntradaSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!nuevaEntrada.numeroLote || !nuevaEntrada.vencimiento) {
      addToast('Ingrese número de lote y fecha de vencimiento válidos', 'warning');
      return;
    }

    const art = articulos.find((a) => a.id === nuevaEntrada.articuloId);

    setArticulos((prev) =>
      prev.map((a) => {
        if (a.id === nuevaEntrada.articuloId) {
          return {
            ...a,
            stockActual: a.stockActual + Number(nuevaEntrada.cantidad),
            loteActivo: nuevaEntrada.numeroLote,
            vencimiento: nuevaEntrada.vencimiento
          };
        }
        return a;
      })
    ...
    createItem('movimientosFarmacia', {
      numeroTransaccion: `ENT-${Date.now().toString().slice(-6)}`,
      tipoMovimiento: 'Entrada / Compra',
      medicamentoNombre: art?.nombreGenerico || 'Medicamento',
      cantidad: Number(nuevaEntrada.cantidad),
      lote: nuevaEntrada.numeroLote,
      fechaVencimiento: nuevaEntrada.vencimiento,
      responsableNombre: nuevaEntrada.responsable,
      fechaHora: new Date().toISOString().replace('T', ' ').substring(0, 16),
      motivo: `Dotación recibida según guía ${nuevaEntrada.guia}`
    });

    addToast(`Entrada de ${nuevaEntrada.cantidad} unidades registrada correctamente bajo política FEFO`, 'success');
