...
export const RecepcionRecipe: React.FC = () => {
  ...
  const agregarMedicamento = () => {
    ...
    if (!med || !nuevoItem.posologia) {
      addToast('Ingrese la posología del medicamento a prescribir', 'warning');
      return;
    }
    ...
    addToast(`Medicamento ${med.nombre} agregado al récipe`, 'info');
    setNuevoItem({ ...nuevoItem, posologia: '', cantidad: 1 });
  };

  const eliminarItem = (id: string) => {
    setItemsPrescritos(itemsPrescritos.filter((i) => i.id !== id));
    addToast('Medicamento eliminado del récipe', 'info');
  };

  const handleGuardarRecipe = (e: React.FormEvent) => {
    e.preventDefault();
    if (itemsPrescritos.length === 0) {
      addToast('Debe incluir al menos un medicamento prescrito en el récipe', 'warning');
      return;
    }
    addToast(`Récipe médico ${recipeHeader.numeroRecipe} recibido e ingresado a la cola de dispensación`, 'success');
  };
