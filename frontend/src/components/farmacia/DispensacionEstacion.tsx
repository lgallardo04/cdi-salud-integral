...
export const DispensacionEstacion: React.FC = () => {
  ...
  const handleBuscar = (e: React.FormEvent) => {
    e.preventDefault();
    if (!criterioBusqueda) {
      addToast('Ingrese número de récipe o cédula del paciente', 'warning');
      return;
    }
    addToast(`Búsqueda completada: Récipe localizado para ${recipeActivo?.paciente || 'paciente'}`, 'info');
  };

  const handleDispensar = () => {
    if (!recipeActivo) return;

    recipeActivo.medicamentos.forEach((m) => {
      createItem('movimientosFarmacia', {
        numeroTransaccion: `DISP-${Date.now().toString().slice(-6)}`,
        tipoMovimiento: 'Salida / Despacho',
        medicamentoNombre: m.nombre,
        cantidad: m.aDespachar,
        lote: m.loteSugerido,
        fechaVencimiento: m.vencimientoLote,
        pacienteNombre: recipeActivo.paciente,
        responsableNombre: personaRetira.farmaceuta,
        fechaHora: new Date().toISOString().replace('T', ' ').substring(0, 16),
        motivo: `Dispensación según orden ${recipeActivo.numeroRecipe}`
      });
    });

    addToast(
      `Dispensación completada para ${personaRetira.nombre}. Descarga FEFO registrada en Kardex.`,
      'success'
    );
  };
